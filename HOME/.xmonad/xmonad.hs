...
-- Colors and fonts
myFont               = "Droid Sans:size=14"
dzenFont             = "Droid Sans:size=9"
colorBlack           = "#000000" --Background (Dzen_BG)
colorBlackAlt        = "#404040" --Black Xdefaults
colorGray            = "#999999" --Gray       (Dzen_FG2)
colorGrayAlt         = "#161616" --Gray dark
colorWhite           = "#808080" --Foreground (Shell_FG)
colorWhiteAlt        = "#1dd" --White dark (Dzen_FG)
colorMagenta         = "#8e82a2"
colorBlue            = "#87afd7"
colorYellow          = "#87ff00"
colorRed             = "#d75f5f"
...
myWorkspaces = ["[1:TERM]", "[2:WEB]", "[3:CODE]", "[4:GRFX]", "[5:CHAT]", "[6:GAME]", "[7:VIDS]", "[8:OTHR]"]
...
-- Layouts
myTile = named "T" $ spacing 1 $ smartBorders $ ResizableTall 1 0.03 0.5 []
myMirr = named "MT" $ spacing 1 $ smartBorders $ Mirror myTile
myMosA = named "M"  $ spacing 1 $ smartBorders $ MosaicAlt M.empty
myObig = named "O"  $ spacing 1 $ smartBorders $ OneBig 0.75 0.65
myTabs = named "TS" $ spacing 1 $ smartBorders $ tabbed shrinkText myTabTheme
myFull = named "TS" $ spacing 1 $ smartBorders $ tabbedAlways shrinkText myTabTheme
myTabM = named "TM" $ spacing 1 $ smartBorders $ mastered 0.01 0.4 $ tabbed shrinkText myTabTheme
myGimp = named "G"  $ spacing 1 $ withIM (0.15) (Role "gimp-toolbox") $ reflectHoriz $ withIM (0.20) (Role "gimp-dock") myMosA
myChat = named "C"  $ spacing 1 $ withIM (0.20) (Title "Buddy List") $ Mirror $ ResizableTall 1 0.03 0.5 []
...
myWorkspaceBar    = "dzen2 -x '0' -y '0' -h '17' -w '1366' -ta 'l' -fg '" ++ colorWhiteAlt ++ "' -bg '" ++ colorBlack ++ "' -fn '" ++ dzenFont ++ "' -p -e ''"
myBottomStatusBar = "conky"
myTopStatusBar    = "conky -c /home/body/.conkydzentop | dzen2 -x '768' -y '0' -h '17' -w '768' -fg '" ++colorWhiteAlt ++ "' -bg '" ++ colorBlack ++ "' -fn '" ++ dzenFont ++ "' -ta r -e ''"
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
    ...
    , ((modMask .|. shiftMask, xK_w), spawn "google-chrome-stable")                                  --Launch chromium
    , ((mod1Mask .|. shiftMask, xK_e), spawn "subl")                                           --Launch chromium
    , ((mod1Mask .|. shiftMask, xK_f), spawn "thunar")                                         --Launch thunar
    , ((modMask .|. shiftMask, xK_t), spawn "termite")                                          --Launch urxvt terminal
    , ((modMask, xK_c), kill)                                                                  --Close focused window
    , ((modMask, xK_o), spawn "dmenu_run -nb '#000000' -nf '#87ff00' -sb '#11ff00' -sf '#000000' -fn 'Tewi' -w 400 -h 20 -y 200 -x 440 ")						       --Launch dmenu
   ...
   -- , ((mod1Mask, xK_s), spawn "shutdown -h now")					       --Shutdown
    ...
    , ((modMask , xK_d), spawn "killall dzen2, conky")                                                --Kill dzen2 and trayer
    ...
    , ((0, xF86XK_AudioMute), spawn "sh /home/body/.xmonad/voldzen.sh t -d")                     --Mute/unmute volume
    , ((0, xF86XK_AudioRaiseVolume), spawn "sh /home/body/.xmonad/voldzen.sh + -d")              --Raise volume
    , ((0, xF86XK_AudioLowerVolume), spawn "sh /home/body/.xmonad/voldzen.sh - -d")              --Lower volume
    ...
    , ((0, xF86XK_MonBrightnessUp), spawn "sh /home/body/.xmonad/bridzen.sh")                    --Raise brightness
    , ((0, xF86XK_MonBrightnessDown), spawn "sh /home/body/.xmonad/bridzen.sh")                  --Lower brightness
    ...
    ++
    [((m .|. mod1Mask, key), screenWorkspace sc >>= flip whenJust (windows . f))                --Switch to n screens and send client to n screens
